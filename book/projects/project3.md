...
Then let's consider the movement in the $xy$-plane. If we further assume $x_0, v_0 > 0$, as we will use later, the specific solution for $f(t)$ is given by eq. {eq}`f_general_sol` with 
...
```{note}
For general initial conditions $x(0) = x_0$, $y(0) = y_0$, $\dot{x}(0) = v_{x,0}$, $\dot{y}(0) = v_{y,0}$,
the amplitudes $A_\pm$ and phases $\phi_\pm$ are the moduli and (negative) arguments of these two complex numbers
...
with $f(0) = x_0 + i y_0$ and $\dot{f}(0) = v_{x,0} + i v_{y,0}$.
```
