...
### Other ideas
- A section on vector vs. array -- thinking mathematically vs. element-wise
    - Could include built-in math functions like `A.array().sin()`
- Basic vector math, e.g. `.dot()`, `.cross()`, `.transpose()`
- Should things like `Eigen::VectorXd::LinSpaced` come in?
- Things like `.norm()`, `.mean()`, `.maxCoeff()` etc.?
- A suggested (complicated) function for writing to binary .npz file?
