...
int main()
{
  ...
  // Here we declare and assign value to a variable 's' 
  // living in the outer scope.
  int s = 10;
  ...
  if (true)
  {
    ...
    // While inside this local scope, the local variable 's' will 
    // take precedence over the 's' variable from the outer scope.
    // So the following line will print 'Hello!' to screen.
    std::cout << s << std::endl;
    ...
    // was of type int. But since the local 's' takes precedence, what
    ...
    // will then no longer contain the value "Hello!".
    s = 20;
  ...
  // Now we are back in the outer scope. The string variable 's' that existed inside the 
  // if block no longer exists. So there is now only one 's' variable, namely our 
  // original integer-type 's', which still has value 10. We will now print this to screen.
  std::cout << s << std::endl;
