...
An example with OpenMP parallelization, where each thread
gets its own random number generator with its own unique seed
...
Each thread gets its own RandomWalker, and the <random>  number generater in 
each RandomWalker is seeded with a seed that is unique to the thread.
